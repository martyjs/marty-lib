...
describe('when', function () {
  ...
  describe('when I pass in a function context as the second argument', function () {
    ...
    beforeEach(function () {
      ...
      Component = TestUtils.renderIntoDocument(React.createElement(React.createClass({
  ...
  describe('#all()', function () {
    ...
    describe('when you pass in an array of fetch results and handlers', function () {
      describe('when one of the fetch results has failed', function () {
        beforeEach(function () {
          when.all([fetch.done(), fetch.pending(), fetch.failed(expectedError)], handlers);
        });
      ...
      describe('when all of the fetch results are done', function () {
        beforeEach(function () {
          when.all([fetch.done(expectedResult1), fetch.done(expectedResult2)], handlers);
        });
  ...
  describe('#join()', function () {
    ...
    describe('when one of the fetch results has failed', function () {
      beforeEach(function () {
        when.join(fetch.done(), fetch.pending(), fetch.failed(expectedError), handlers);
      });
    ...
    describe('when all of the fetch results are done', function () {
      beforeEach(function () {
        when.join(fetch.done(expectedResult1), fetch.done(expectedResult2), handlers);
      });
